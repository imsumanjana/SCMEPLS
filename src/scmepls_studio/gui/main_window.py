...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._build_menu()
        status = QStatusBar(); status.showMessage(DISCLAIMER); self.setStatusBar(status)

    def _build_menu(self) -> None:
        file_menu = self.menuBar().addMenu("File")
        batch_action = QAction("Export All Current Article Figures…", self)
        batch_action.triggered.connect(self.export_all)
        file_menu.addAction(batch_action)
        file_menu.addSeparator()
        exit_action = QAction("Exit", self); exit_action.triggered.connect(self.close); file_menu.addAction(exit_action)
        help_menu = self.menuBar().addMenu("Help")
        about_action = QAction("About", self); about_action.triggered.connect(self.about); help_menu.addAction(about_action)
    ...
    def export_all(self) -> None:
        ...
        try:
            ...
            if not self.energy_tab.comparison.empty:
                self.energy_tab.comparison.to_csv(out / "energy_metrics.csv", index=False)
            if not self.radar_tab.ranked.empty:
                self.radar_tab.ranked.to_csv(out / "radar_weighted_ranking.csv", index=False)
            if not self.rollout_tab.df.empty:
                self.rollout_tab.df.to_csv(out / "scmepls_time_history.csv", index=False)
            QMessageBox.information(self, "Batch export complete", f"All current figures and result tables were saved to:\n{out}")
        except Exception as exc:
            QMessageBox.critical(self, "Batch export error", str(exc))

    def about(self) -> None:
        QMessageBox.information(
            self,
            "About SC-MEPLS Analysis Studio",
